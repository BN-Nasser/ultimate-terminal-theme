...
local colors = {
   ...
   close_default         = { bg = '#45475A', fg = '#6c7086' },
   close_hover           = { bg = '#7188b0', fg = '#f38ba8' },
   close_active          = { bg = '#89b4fa', fg = '#f38ba8' },
...
local function create_base_title(pane_title, process_name)
   ...
   -- Determine Prefix Icon based on process
   ...
   elseif ustr.starts_with(base_title, 'Password:') then
      prefix_icon = ICON_PREFIX.password
   elseif ustr.starts_with(process_name, 'wsl') then
      prefix_icon = ICON_PREFIX.wsl
   elseif process_name:match('nvim') or process_name:match('vim') then
      prefix_icon = nf.custom_vim or nf.md_code_braces
   elseif process_name:match('lua') then
      prefix_icon = nf.md_language_lua
   elseif process_name:match('python') or process_name:match('py') then
      prefix_icon = nf.md_language_python
   elseif process_name:match('git') then
      prefix_icon = nf.dev_git_branch
   elseif process_name:match('node') or process_name:match('npm') then
      prefix_icon = nf.md_language_javascript
   elseif ustr.starts_with(base_title, 'InputLine:') then
      prefix_icon = ICON_PREFIX.edit
      base_title = base_title:gsub('InputLine: ', '')
   else
      prefix_icon = nf.linux_penguin or nf.cod_terminal
   end
...
local function create_title(process_name, base_title, max_width, inset)
   ...
   local target_content_width = max_width - inset

   if current_width > target_content_width then
      title = wezterm.truncate_right(title, target_content_width)
   else
      -- Pad with non-breaking spaces to force full width rendering
      local padding_len = target_content_width - current_width
      title = title .. string.rep('\u{00A0}', padding_len)
   end
...
M.setup = function(opts)
   ...
   -- Chrome/VS Code-style tab width strategy
   local _PREFERRED = 28  -- ideal tab width when space is plentiful
   local _MIN       = 12  -- hard floor: icon + a few title chars
   local _cache     = { num_tabs = 0, window_width = 0, left_width = 0, per_tab = _PREFERRED }

   wezterm.on('format-tab-title', function(tab, tabs, panes, config, hover, _max_width)
      if not tab_list[tab.tab_id] then
         tab_list[tab.tab_id] = Tab:new()
      end

      local window_width = wezterm.GLOBAL.window_width or 220
      local status_width = wezterm.GLOBAL.status_width or 60
      local left_width   = wezterm.GLOBAL.workspace_width or 12
      local num_tabs     = math.max(1, #tabs)

      -- Only recalculate when layout inputs actually change
      if num_tabs ~= _cache.num_tabs
         or window_width ~= _cache.window_width
         or left_width ~= _cache.left_width
      then
         local available = window_width - status_width - left_width - 2

         if num_tabs * _PREFERRED <= available then
            -- Phase 1: plenty of room — use preferred width
            _cache.per_tab = _PREFERRED
         elseif num_tabs * _MIN <= available then
            -- Phase 2: tight — shrink evenly, never below minimum
            _cache.per_tab = math.floor(available / num_tabs)
         else
            -- Phase 3: overflow — clamp at minimum, titles truncate
            _cache.per_tab = _MIN
         end

         _cache.num_tabs = num_tabs
         _cache.window_width = window_width
         _cache.left_width = left_width
      end

      tab_list[tab.tab_id]:update_cells(valid_opts, tab, hover, _cache.per_tab)
